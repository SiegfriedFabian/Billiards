...
// wir �bergeben VAO und fbo von einem Rectangle und kopieren dieses Rechtangle dann hierher, aber das kopierte rect hat doch gar nichts mit dem Shader zu tun oder?
// wie wird hier festgelegt, wo auf dem Bildschirm gemalt werden soll? Weil wenn Fenster gro�, dann ist Bild gr��er als Fenster und wenn Fenster klein, dann ist Bild kleiner als Fenster

// neue Beobachtung:
// ich wei� nicht, was ich angestellt habe, aber das Bild wird jetzt komisch zuerst in der Ecke aufgebaut und dann auf den kompletten Schirm �bertragen????
// ich kann nichtmal mit Git finden, was ich ge�ndert habe, dass daf�r sorgt? 
...
void refreshPhasespace(FourthBilliard billiard,int numberOfDistinctVertices, int n_iter, GLuint VAO, GLuint fbo, Shader shaderprogram, Shader shaderprogramTex, Rectangle rect, GLFWwindow* window){
	...
	for (int i = 0; i < grid; i++)
	{
		for (int j = 0; j < grid; j++)
		{
...
#else
			glUniform1f(glGetUniformLocation(shaderprogram.ID, "width"), SCR_WIDTH);	// warum setzen wir die uniforms so und nicht �ber .setUniform wie den Rest?
			...
			glClear(GL_COLOR_BUFFER_BIT);
			// Tell OpenGL which Shader Program we want to use
			shaderprogramTex.Activate();																	// was macht das hier?, wenn ich es auskommentiere �ndert sich nichts//
#if __APPLE__
